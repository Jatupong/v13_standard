...
class account_move(models.Model):
    ...
    def _compute_amount(self):
        ...
        for move in self:
            ...
            for line in move.line_ids:
                ...
                if move.is_invoice(include_receipts=True):
                    ...
                    if not line.exclude_from_invoice_tab:
                        ...
                        debit_tax = move.discount_value * 0.07 ### add by JA
                        print ('---Update Tax')
                        print (debit_tax)
                        print(line.credit)
                        print(line.balance)
                        line.credit = line.credit - debit_tax #### Modify by JA
                        line.amount_currency = line.amount_currency - debit_tax #### Modify by JA
                        print (line.credit)
                        print(line.balance)
                        ...
                        value = 0
                        total_value = 0
                        if self._context.get('default_type') in ('in_invoice','in_receipt'):
                            ...
                            for rec in move.line_ids:
                                if rec.discount_line == True:
                                    ...
                                    discount_amt = res
                                    print ('----------Applyy Discount Bill----------')
                                    print (total_value) #1000
                                    print (value)       #0
                                    print (discount_amt)#100
                        ...
                        elif self._context.get('default_type') in ('out_invoice','out_receipt'):
                            for val in move.line_ids:
                                if not val.exclude_from_invoice_tab:
                            ...
                            for rec in move.line_ids:
                                if rec.discount_line == True:
                                    ...
                                    discount_amt = res
                                    rec.debit = discount_amt
                                    print('----------Applyy Discount Invoice----------')
                                    print(total_value)
                                    print(value)
                                    print(discount_amt)
                                    rec.name = move.discount_type_id.name
                                    # line.debit = total_value + value - rec.debit
                                    line.debit = total_value + abs(total_tax) - rec.debit
            ...
            print ('--AMOUNT TAX--')
            print (total_tax)
            print (total_tax_currency)
            # move.amount_tax = sign * (total_tax_currency if len(currencies) == 1 else total_tax)
            move.amount_tax = sign * total_tax ############# by JA
        ...
        if self.apply_discount:
            ...
            for line in self.line_ids:
                if count==1:
                    ...
                    if self._context.get('default_type') in ['out_invoice']:
                        discount_vals = {
                            ...
                            'name': self.discount_type_id.name,
                            'exclude_from_invoice_tab': True,
                            'tax_ids': [(6, 0, [2])], ### add by JA
                            'tag_ids': [(6, 0, [10])], ### add by JA
                            'discount_line': True,
                        }

                    if self._context.get('default_type') in ['in_invoice']:
                        discount_vals = {
    ...
    def create(self, vals_list):
        for val in vals_list:
            ...
            if res.apply_discount and (not res.discount_type_id or not res.discount_value):
                raise Warning(_('Please give the Discount Type and its values'))
            if res.apply_discount == True:
                ...
                if self._context.get('default_type') in ('out_invoice','out_receipt','in_refund'):
                ...
                elif self._context.get('default_type') in ('in_invoice','in_receipt','out_refund'):
    ...
    def onchange_type(self):
        if self._context.get('default_type') in ('in_invoice','in_receipt','out_refund'):
            for line in self.line_ids:
                name = line.name
                if name == False or name == '':
                    line.debit = 0.0
    ...
    def write(self,vals):
        ...
        if self.apply_discount and (not self.discount_type_id or not self.discount_value):
            raise Warning(_('Please give the Discount Type and its values'))
        return res
