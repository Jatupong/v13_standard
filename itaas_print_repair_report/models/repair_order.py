...
class RepairOrder_inherit(models.Model):
    _inherit ="repair.order"

    # child_ids = fields.One2many(string='Add Customer')
    analytic_account_id =fields.Many2one('account.analytic.account',string="Analytic Account")
    analytic_tag_id =fields.Many2one('account.analytic.tag',string="Analytic Tag")

    #
    # normal = fields.Boolean(string='ปกติ')
    # defective = fields.Boolean(string='ผิดปกติ')
    #
    # without = fields.Boolean(string='ไม่มี')
    # cost = fields.Boolean(string='มี')
    # price = fields.Float(string='ราคา')
    #
    # repairable = fields.Boolean(string='ซ่อมได้')
    # cannot_fixing = fields.Boolean(string='ซ่อมไม่ได้')
    # claim_product = fields.Boolean(string='เคลมสินค้า')
    # another = fields.Boolean(string='อื่นๆ')
    #
    # reported_symptoms = fields.Text(string='Note Reported')
    # cause_problem = fields.Text(string='Note Cause of problem')
    # method = fields.Text(string='Note How to fix the problem')
    # note_repair = fields.Text(string='Note')
    # data_time = fields.Text(string='ครั้งที่')
    #
    # hour =fields.Boolean(string='12 ชั่วโมง')
    # hour_1 = fields.Boolean(string='24 ชั่วโมง')
