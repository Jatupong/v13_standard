...
class SaleOrder(models.Model):
    ...
    current_revision_id = fields.Many2one('sale.order', string='Current revision', readonly=True,copy=True)
    old_revision_ids = fields.One2many('sale.order', 'current_revision_id', string='Old revisions', readonly=True,
    ...
    def write(self, values):
        if 'name' in values and 'unrevisioned_name' not in values and 'state' not in ['draft','sent']:
            values.update({'unrevisioned_name':values['name']})
