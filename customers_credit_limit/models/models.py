...
class customers_credit_limit(models.Model):
    ...
    so_draft_invoices_count = fields.Integer(
        ...
        string="Total Invoices:") ################ old name is draft but it is not draft any more, it is invoice which not paid yet.
    so_draft_invoices_total = fields.Float(
        compute='_count_draft_invoices',
        string="Invoices Amount:") ################ old name is draft but it is not draft any more, it is invoice which not paid yet.
    ...
    ################ old name is draft but it is not draft any more, it is invoice which not paid yet.
    ################ JA-25/08/2020 ############3
    def _count_draft_invoices(self):
        ...
        draft_invoices = self.env['account.move'].search(
            [('partner_id.commercial_partner_id', '=', ptnr.id),
             ('state', '=', 'posted'), ('invoice_payment_state','=','not_paid'), ('type', '=', 'out_invoice')])
    ...
    def _count_overdue_invoices(self):
        ...
        # [('partner_id.commercial_partner_id', '=', ptnr.id), ('state', '=', 'posted'), ('type', '=', 'out_invoice')])
        overdue_invoices = self.env['account.move'].search([('partner_id.commercial_partner_id','=',ptnr.id),('state','=','posted'),('invoice_payment_state','=','not_paid'),
                                                            ('type','=','out_invoice'),
                                                            ('invoice_date_due','<=',fields.Date.to_string(datetime.now() -timedelta(days=1)))])

        print ('--------_count_overdue_invoices---')
        print (fields.Date.to_string(datetime.now() -timedelta(days=1)))
